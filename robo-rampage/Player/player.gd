...
func _physics_process(delta: float) -> void:
	...
	if not is_on_floor():
		if velocity.y >= 0:
			...
			velocity.y -= gravity3d * delta * fall_multiplier

	# Handle jump.
	if Input.is_action_just_pressed("jump") and is_on_floor():
		velocity.y = sqrt(jump_height * 2.0 * gravity3d)

	# Get the input direction and handle the mov ice, you should replace UI actions with custom gameplay actions.
